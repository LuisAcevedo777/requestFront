...
export const getRequestThunk = () => async(dispatch) => {
  ...
  await axios
    .get("https://requestserver-1.onrender.com/api/request", {
      headers: { token: token },
    })
    .then((res) => {
...
export const filterRequestTitleThunk = (title,role,employeeId) => async(dispatch) => {
 ...
 if(role==="admin"){
    ...
    .finally(() => dispatch(setIsLoading(false)));}
        ...
        await axios
     .get(`https://requestserver-1.onrender.com/api/employee/${employeeId}`, {
       headers: { token: token },
     })
     .then((res) => {
...
export const requestEmployeeThunk = (id) => async(dispatch) => {
    dispatch(setIsLoading(true));
    await axios
    .get(`https://requestserver-1.onrender.com/api/employee/${id}`, {
      headers: { token: token }
    })
    .then((res) => {
